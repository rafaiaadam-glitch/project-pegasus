...
cat > "$TMP_DIR/fake_canary_server.py" <<'PY'
import json
import os
...
jobs = {
    "job-transcribe": ["queued", "running", "succeeded"],
    "job-generate": ["queued", "running", "succeeded"],
    "job-export": ["queued", "running", "succeeded"],
}
job_polls = {k: 0 for k in jobs}

lecture_id = "canary-lecture-http"
state = {
    "webhook": None,
    "deleted": False,
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == f"/exports/{lecture_id}/markdown":
            self._write_json(200, {"lectureId": lecture_id, "exportType": "markdown", "content": "# Canary"})
            return
    ...
    def do_POST(self):
        ...
        if self.path == f"/lectures/{lecture_id}/generate":
            self._write_json(200, {"jobId": "job-generate", "status": "queued"})
            return

        if self.path == f"/lectures/{lecture_id}/export":
            self._write_json(200, {"jobId": "job-export", "status": "queued"})
            return
...
PORT_FILE="$PORT_FILE" STATE_FILE="$STATE_FILE" python "$TMP_DIR/fake_canary_server.py" >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
...
if [[ ! -s "$PORT_FILE" ]]; then
  echo "FAIL: fake canary server did not start" >&2
  exit 1
fi

PORT="$(cat "$PORT_FILE")"
API_BASE_URL="http://127.0.0.1:$PORT" \
CANARY_LECTURE_ID="canary-lecture-http" \
...
assert summary['jobs']['generate'] == 'job-generate', summary
assert summary['jobs']['export'] == 'job-export', summary

state = json.loads(Path('$STATE_FILE').read_text())
webhook = state['webhook']
assert webhook is not None, state
assert webhook['status'] == 'passed', webhook
assert webhook['lectureId'] == 'canary-lecture-http', webhook
assert state['deleted'] is True, state
print('validation-ok')
PY

echo "Canary HTTP simulation passed"
