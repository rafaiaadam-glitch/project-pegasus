            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    insert into deletion_audit_events (
                        id, entity_type, entity_id, action, purge_storage,
                        actor, request_id, deleted_counts, created_at
                    ) values (
                        %(id)s, %(entity_type)s, %(entity_id)s, %(action)s, %(purge_storage)s,
                        %(actor)s, %(request_id)s, %(deleted_counts)s, %(created_at)s
                    );
                    """,
                    {
                        **payload,
                        "deleted_counts": Json(payload.get("deleted_counts") or {}),
                    },
                )
    ...
    def fetch_deletion_audit_events(
        ...
        with self.connect() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    f"""
                    select * from deletion_audit_events{where_clause}
                    order by created_at desc{limit_clause};
                    """,
                    params,
                )
