...
- Flashcards
- Exam-style questions with answer validation, scoring, and explanations

Across multiple lectures, PLC tracks:
- Repeated concepts with cross-lecture attribution
- Refinements and contradictions
- Foundational vs advanced ideas
- Connections between lectures in the same course
...
- Loads existing threads from the database when processing new lectures, enabling real cross-lecture connections in production (Cloud Run)
...
Exam questions include answer validation, scoring, and explanations:
- Multiple-choice answers are checked against `correctChoiceIndex` with green/red feedback
- Every question includes an `explanation` field generated by the LLM
- Results screen shows score (correct/total) and accuracy percentage

### Cross-Lecture Connections

When generating artifacts for a new lecture, the Thread Engine loads all existing threads for the course from the database. This means:
- Concepts from earlier lectures are detected and updated (not duplicated)
- The LLM sees which lectures each concept appeared in
- Thread continuity works in production (Cloud Run) without local file persistence
